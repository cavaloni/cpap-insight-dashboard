...
// Tool 7: Analyze Best Sleep
export function analyzeBestSleep(
  dateRange: { start: string; end: string },
  topN: number = 5
): ToolResult<BestSleepAnalysis> {
  const db = getDatabase();
  const artifactId = uuidv4();

  const summaries = getNightlySummary(dateRange);
  const nights = summaries.data
    .filter(n => n.sleep_quality_score !== null && n.sleep_quality_score !== undefined)
    .slice();

  nights.sort((a, b) => (b.sleep_quality_score || 0) - (a.sleep_quality_score || 0));
  const bestNights = nights.slice(0, Math.max(1, Math.min(topN, nights.length)));

  const candidateMetrics: MetricName[] = [
    'ahi',
    'total_usage_minutes',
    'mask_on_minutes',
    'median_pressure',
  ...
  const correlationStmt = (metric: MetricName) => db.prepare(`
    SELECT sleep_quality_score as valueA, ${metric} as valueB
  ...
  for (const metric of candidateMetrics) {
    const rows = correlationStmt(metric).all(dateRange.start, dateRange.end) as { valueA: number; valueB: number }[];
    const corr = calculateCorrelation(rows.map(r => r.valueA), rows.map(r => r.valueB));
    ...
    if (corr > 0.1) direction = 'positive';
    else if (corr < -0.1) direction = 'negative';

    correlations.push({
      metricA: 'sleep_quality_score',
      metricB: metric,
      correlation: corr,
      strength,
      direction,
      interpretation:
        direction === 'positive'
          ? `Higher ${METRIC_DEFINITIONS[metric].name.toLowerCase()} tends to align with higher sleep quality score.`
          : direction === 'negative'
            ? `Higher ${METRIC_DEFINITIONS[metric].name.toLowerCase()} tends to align with lower sleep quality score.`
            : `No clear relationship found between ${METRIC_DEFINITIONS[metric].name.toLowerCase()} and sleep quality score.`
    });
  }

  correlations.sort((a, b) => Math.abs(b.correlation) - Math.abs(a.correlation));

  const result: ToolResult<BestSleepAnalysis> = {
    data: {
      dateRange,
      topN: bestNights.length,
      bestNights,
      correlations
    },
    provenance: {
      toolName: 'analyzeBestSleep',
      parameters: { dateRange, topN },
      computedAt: new Date().toISOString(),
      definitions: [
        'Best nights are selected by highest sleep_quality_score in nightly_aggregates',
        `Top N: ${bestNights.length}`,
        'Correlations are Pearson correlations computed over the full date range (not just best nights)',
  ...
  const result: ToolResult<NightlySummary[]> = {
    ...
    provenance: {
      ...
      definitions: [
        ...
        `Leak Rate: ${METRIC_DEFINITIONS.median_leak_rate.description} (${METRIC_DEFINITIONS.median_leak_rate.unit})`,
...
export function getTrends(
  ...
  if (metric === 'ahi' || metric === 'median_leak_rate' || metric === 'large_leak_percent') {
...
export function detectAnomalies(
  ...
  for (const result of results) {
    ...
    if (zScore > threshold) {
      ...
      } else if (metric === 'median_leak_rate' || metric === 'large_leak_percent') {
